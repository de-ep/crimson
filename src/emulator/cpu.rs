...
use super::{decoder::{self, Inst, InstType}, Emulator};
...
pub fn exec(emu: &mut Emulator, inst: Inst, inst_type: Option<InstType>) -> Result<(), CpuErr> {
    let mut inst = inst;
    let inst_type = inst_type.unwrap_or_else(|| {
            inst = Inst::Undefined;
            InstType::R
        }
    );
